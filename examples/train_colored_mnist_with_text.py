...
train_dataset = ColoredMNISTWithTextDataset(split="train", tokenizer=tokenizer)
test_dataset = ColoredMNISTWithTextDataset(split="test", tokenizer=tokenizer)

print("\t\tNumber of tokens:", len(tokenizer))

experiment_dir = EXPERIMENT_DIR / "colored_mnist_images_with_text"
...
trainer = TrainerAutoregression(
    ...
    generate_samples_fn=cast(
        GenerateSamplesProtocol, generate_samples_colored_mnist_with_text
    ),
    show_plots=False,
