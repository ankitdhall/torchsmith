...
class VQVAEColoredMNISTImageTokenizer(BaseTokenizer[TokenType]):
    ...
    def __init__(
        ...
        *,
        ...
        self.vqvae = load_pretrain_vqvae().eval().to(self.device)
        self._id_to_token: dict[int, TokenType] = {
            (index + token_id_offset): token
            for index, token in enumerate(range(self.vqvae.n_embeddings))
        }
    ...
    def image_to_sequence(self, x: np.ndarray) -> np.ndarray:
        with torch.no_grad():
            return (
                cast(torch.Tensor, self.vqvae.quantize(x).reshape(x.shape[0], -1))
                .cpu()
                .numpy()
            )
    ...
    def decode(self, sequence: list[int]) -> np.ndarray:
        ...
        with torch.no_grad():
            return self.vqvae.decode(sequence_2d)
...
class ColoredMNISTImageAndTextTokenizer(BaseTokenizer):
    ...
    def __init__(
        ...
        *,
        ...
        self.image_tokenizer = VQVAEColoredMNISTImageTokenizer(
